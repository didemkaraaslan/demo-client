...
import { Grid, Header, Menu, Label, Image, Checkbox } from 'semantic-ui-react';
...
const ProductsList = ({ categories }) => {
  ...
  const filterProductsByCategory = () => {
    if(activeMenuItem === 'Hepsi') {
      const selectedCategoryNames = categories.map(category => category.categoryName);
      return  products.filter(product => selectedCategoryNames.includes(product.productCategoryName));
    }

    return products.filter(product => product.productCategoryName === activeMenuItem)
  };
  ...
  useEffect(() => {
    socket.on(EVENT_DELETE_PRODUCT, productIdToBeDeleted => {
      ...
      setProducts(prevProducts =>
        prevProducts.filter(product => product.productId !== productIdToBeDeleted));
    });
  }, []);
  ...
  return(
    <Grid>
      <Grid.Column width={3}>
        <Menu vertical  inverted borderless fixed="left" style={{ minHeight: 500 }}>
      ...
      <Grid.Column width={13}>
        <Header as='h3' textAlign='center' style={{ marginTop: 15, marginBottom: 25 }}> Ürünler </Header>
